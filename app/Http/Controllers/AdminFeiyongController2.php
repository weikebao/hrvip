...
class AdminFeiyongController extends Controller {
	...
	public function postXiangxiajax(Request $request)
	{
		...
		if($leixing==1){
			//交易记录
			$res = DB::table('hongri_orders')->where('phone',$phone)->where('status','>=',1)->where('paytime','like','%'.$yue.'%')->orderBy('paytime','desc')->select('hongri_orders.paytime as time','hongri_orders.total as total')->get();
			echo json_encode($res);
		}else if($leixing==2){
			//返利记录
