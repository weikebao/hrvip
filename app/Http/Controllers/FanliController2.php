...
class FanliController extends Controller
{
   //返利控制器  每天都会监听会员是否能够满足返利的要求
			...
			foreach($user as $v){
				...
				if((count($data))>=3){
					...
					foreach($data as $value){
						...
						}else if($sanjiedian!=''||$sanjiedian!=0){
	...
	public function getFanli()
	{
		...
		foreach($user as $v){
			...
			if(strtotime($v['fanliriqi'])>time()){
				...
				$data['yid'] = $id;
				$data['yphone'] = $v['phone'];
				$data['Jtime'] = date('Y-m-d H:i:s',time());//11号的是12号开始计算所以把日期减1天
				//将数据插入到数据库
	...
	{
		$user = DB::table('vip_huiyuan')->where('huiyuanstatus',1)->where('jidian',2)->orwhere('jidian',3)->where('s',1)->get();
			foreach($user as $v){
	...
	{
		$user = DB::table('vip_huiyuan')->where('huiyuanstatus',1)->where('jidian','>',1)->where('s',0)->get();
			foreach($user as $v){
				//这个是将90天内没有进行购买商品的用户删除
				if((time()-strtotime($v['firstgoumai'])>=30*24*3600*3)){
					// 讲这个用户的信息删除掉保留这个位置
					$data['phone']='';
					$data['pass']='';
					$data['name'] = '';
